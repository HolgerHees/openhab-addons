...
public final class GraalPythonScriptEngine extends AbstractScriptEngine
                    ...
                    if (value instanceof Boolean) {
                        ...
                        try {
                            ...
                            throw new IllegalArgumentException(String.format(
                                    "failed to set graal-js option \"%s\": expected a boolean or Predicate<String> value, got \"%s\"",
                                    getOptionKey(), value));
                        }
                    }
                }
    ...
    GraalPythonScriptEngine(GraalPythonScriptEngineFactory factory, Engine engine, Context.Builder contextConfig) {
        ...
        if (contextConfigToUse == null) {
            // default config
            contextConfigToUse = Context.newBuilder(ID) //
                    .allowExperimentalOptions(true) //
                    .allowIO(IOAccess.newBuilder().allowHostSocketAccess(true).fileSystem(null).build()) //
                    ...
                    .option(PYTHON_OPTION_EXECUTABLE, PYTHON_DEFAULT_PATH.toString() + "/lib") //
                    .option(PYTHON_OPTION_PYTHONHOME, PYTHON_DEFAULT_PATH.toString() + "/home") //
                    .option(PYTHON_OPTION_PYTHONPATH, PYTHON_DEFAULT_PATH.toString()) //
                    .option(PYTHON_OPTION_INPUTFILEPATH, "") //
                    .engine(engine);
